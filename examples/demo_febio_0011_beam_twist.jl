...
F2 = Fb[Cb.==3]
indNodesFront = elements2indices(F2)

V2_centre = mean(V[indNodesFront])
...
ax1 = AxisGeom(fig[1, 1], title="Hexahedral mesh")
hp11 = meshplot!(ax1, Fbs, Vbs; color=Cbs, colormap=cmap)
hp12 = scatter!(ax1, V, color=:black, markersize=5)
Colorbar(fig[1, 1][1, 2], hp11)
...
hp5 = scatter!(ax2, V[indNodesBack], color=:black, markersize=10)
Legend(fig[1,3], [hp3, hp4, hp5],["Solid mesh", "Rigid body plate", "Supported nodes"])
...
time_stepper_node = aen(control_node,"time_stepper"; type = "default")
    ...
    aen(time_stepper_node,"dtmin",(1.0/numTimeSteps)/1000.0)
    aen(time_stepper_node,"dtmax",1.0/numTimeSteps)
...
Rigid_node = aen(febio_spec_node, "Rigid")
    rigid_bc = aen(Rigid_node,"rigid_bc"; name="RigidFix_end", type="rigid_fixed")
        aen(rigid_bc, "rb", 2)
        aen(rigid_bc, "Rx_dof", 1)
        aen(rigid_bc, "Ry_dof", 1)
        aen(rigid_bc, "Rz_dof", 1)
        aen(rigid_bc, "Ru_dof", 1)
        aen(rigid_bc, "Rv_dof", 0)
...
#######
# Visualization
fig = Figure(size=(1200,1000), fontsize=25)
...
lines!(ax2, time_curve, My, color=:red, linewidth=4)
hp2 = scatter!(ax2, Point{2,Float64}(time_curve[stepStart+1], My[stepStart+1]), markersize=25, color=:red)
